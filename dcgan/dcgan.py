...
class Generator(object):
    ...
    def __init__(self, input, in_num, name='generator'):
        ...
        with tf.variable_scope(name):
            ...
            self.output = tf.nn.sigmoid(deconv5)
...
def img_flat_read(fpass):
    ...
    # 1列にし0-1のfloatに
    img_flat = img_64.reshape([1, INPUT_NUM]) / 255.0
    #cv2.imshow('image', img_64)
    #cv2.waitKey(0)
...
with tf.Session() as sess:
    ...
    for i in range(100000):
        zInput = np.zeros([TRAIN_NUM, 100])
        for j in range(TRAIN_NUM):
            zInput[j] = np.array([(2.0 * np.random.rand() - 1.0) for k in range(100)])
        
        g_optim.run(feed_dict={z:zInput, images:train_x})
        d_optim.run(feed_dict={z:zInput, images:train_x})
        if i % 10 == 0:
            loss_val_d, loss_val_g, img, d_f, d_t  = sess.run([g_loss, d_loss, fake_img, d_fake, d_true], feed_dict={z:zInput, images:train_x})
            #print(d_f)
